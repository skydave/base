...
/*
	...
	inline Vec2f normalize( const Vec2f &vector )
	{
		Vec2f result = vector;
		result.normalize();
		return result;
	...
	inline float dotProduct( const Vec2f &lhs, const Vec2f &rhs )
	{
		return (lhs.x*rhs.x + lhs.y*rhs.y);
	}

	inline void dotProduct( float &result, const Vec2f &lhs, const Vec2f &rhs )
	{
...
/*
	...
	inline unsigned long setColor( const Color &color )
	{
		return setColor(getRed(color), getGreen(color), getBlue(color), getAlpha(color));
	}
...
float                               distancePointSphere( const math::Vec3f &point, const Vec3f &center, const float &radius ); // computes the signed distance between a 3d point and a sphere given by its center and radius (negative distance means inside)
float      distancePointCylinder( const math::Vec3f &point, const Vec3f &center1, const Vec3f &center2, const float &radius ); // computes the signed distance between a 3d point and a cylinder given by its centers and radius (negative distance means inside)
